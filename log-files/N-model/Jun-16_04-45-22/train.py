...
def add_disc_sum_rew(trajectories, policy, network, gamma, lam, scaler, iteration):
    ...
    for trajectory in trajectories:
        ...
        #     disc_sum_rew = discount(x=trajectory['rewards'],   gamma= gamma, v_last = trajectory['rewards'][-1])
            ...
            advantages = discount(x=trajectory['rewards'],   gamma= gamma, v_last = trajectory['rewards'][-1])
    ...
    if iteration == 1:
        scaler.update(np.hstack((unscaled_obs, advantages))) # scaler updates just once
...
def val_fun_2(trajectories, gamma, scaler):
    """
    ...
    values = np.concatenate([t['value'][:-burn] for t in trajectories])
    scale, offset = scaler.get()
