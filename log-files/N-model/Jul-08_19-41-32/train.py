...
def add_disc_sum_rew(trajectories, policy, network, gamma, lam, scaler, iteration, state1_dict, logger, states = None):
    ...
    for trajectory in trajectories:
        ...
        if iteration!=1:
            ...
            log_vals(disc_sum_rew, str(counter) + 'val1_', states, logger,  True )
...
def advantage_fun(trajectories, policy, network, gamma, lam, scaler, iteration, state_dict, logger, states= None):
    ...
    for trajectory in trajectories:
        ...
        if iteration!=1:
            ...
            lst1 = []
            for i in states:
                logger.log({str(counter) + 'adv2_' + str(i) : list(tds_pi[i])[0],
                            str(counter) +'adv2_full_' + str(i) : list(advantages[i])[0],
                            str(counter) +'adv%' + str(i): (list(tds_pi[i])[0]/ list(advantages[i])[0])})
            ...
            lst1 = []
            for i in states:
                logger.log({str(counter) +'adv2_' + str(i) : list(trajectory['rewards'][i])[0],
                            str(counter) +'adv2_full_' + str(i) : list(advantages[i])[0],
                            str(counter) +'adv%' + str(i):(list(trajectory['rewards'][i])[0] / list(advantages[-i])[0])})
...
def val_fun_2(trajectories, gamma, iteration, scaler, lam, logger, states= None):
    ...
    for trajectory in trajectories:
        ...
        log_vals(value, str(counter) +'val2_', states, logger, True)
...
def build_train_set2(trajectories, gamma, scaler, state_dict, logger, states = None):
    ...
    for trajectory in trajectories:
        ...
        log_vals(advantages, str(counter) + 'adv1_', states, logger, True)
...
if __name__ == "__main__":
    ...
    parser.add_argument('-n', '--num_policy_iterations', type=int, help='Number of policy iterations to run',
                        default=15) #default=50, use 5 for val fun comp.
