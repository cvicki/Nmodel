...
def add_disc_sum_rew(trajectories, policy, network, gamma, lam, scaler, iteration, L):
    ...
    for trajectory in trajectories:
        ...
        print('disc shape ', disc_sum_rew.shape)
    ...
    print('disc norm shape ', disc_sum_rew_norm.shape)
    print('observes ', observes.shape)
...
def advantage_fun(trajectories, gamma, lam, scaler, iteration, val_func, logger, L):
    """
    ...
    for trajectory in trajectories:
        ...
        if iteration!=1:
            ...
            advantages = discount_2(x=summed_vals, gamma= gamma, L = L ) #+ values  #new advantage function
            # advantages = discount(x=summed_vals, gamma= lam*gamma, v_last =  summed_vals[-1]) #new advantage function


        else:
            advantages = discount_2(x=trajectory['rewards'],   gamma= gamma, L= L)
            # advantages = discount(x=trajectory['rewards'], gamma= lam*gamma, v_last = trajectory['rewards'][-1]) #new advantage function
...
def add_disc_sum_rew_2(trajectories, policy, network, gamma, lam, scaler, iteration, val_func):
    """
    for algo 2 method 2 
    compute value function for further training of Value Neural Network
    :param trajectory: simulated data
    ...
    for trajectory in trajectories:
        ...
        if iteration!=1:
            ...
            # td-error computing
            tds_pi = trajectory['rewards'] - values + gamma*values_next#gamma * np.append(values[1:], values[-1]), axis=0)#
        ...
        print('disc_sum ', disc_sum_rew.shape)
...
def build_train_set(trajectories, gamma, scaler):
    ...
    return observes,  actions, advantages, values
    # return act_adv
...
def main(network, num_policy_iterations, no_of_actors, episode_duration, no_arrivals, gamma, lam, clipping_parameter,
    ...
    print(int(1. / sum(network.p_arriving) * no_arrivals))
    ...
    while iteration < num_policy_iterations:
        ...
        if iteration % 8 == 0: #how often save weights 
        ...
        # disc_sum_rew_norm, observes = add_disc_sum_rew_2(trajectories, policy, network, gamma, lam, scaler, iteration, val_func) #algo 1
        ...
        add_value(trajectories, val_func, scaler, network.next_state_list())
        # compute advantage function estimates  
        ####change back to disc_sum_rew in function when using add_disc_sum_rew2 in code
        observes,  actions, advantages, disc_sum_rew = build_train_set(trajectories, gamma, scaler)
    ...
    ################## Policy performance #######################   
    #for 80 iterations, every 8 policies, evaluate
    performance_evolution_all, ci_all = run_weights(network, weights_set, policy, scaler,
                                        time_steps=2* int(1. / sum(network.p_arriving) * no_arrivals)) #no_arrivals = 10^6
