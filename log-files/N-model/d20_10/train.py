...
def adv_logger(adv, trajectory, state_dict, logger):
    """
    Advantage function logger.
    :param values: the records
    :param observes: states 
    :param state_dict: states to log
    For each policy update logs (first time reach state): state, action taken, 
        advantage function estimate for choosing buffer 1 vs buffer 2  
    """
    observes = trajectory['unscaled_obs']
    visited = []

    # print(trajectory['actions'][10])
    # print(trajectory['actions'][10].shape)

    for i in range(len(observes)):
        state = str(observes[i])
        if state_dict.get(state) != None and visited.count(state) == 0:
            visited.append(state)
            curr_state = str(trajectory['unscaled_obs'][i])
            logger.log({curr_state : i,
                        curr_state + '1 action taken': list(trajectory['actions'][i])[0],
                        curr_state + 'adv': list(adv[i])[0]})
            #log the diff advantages: 

            # for j in


# def advantage_fun(trajectories, policy, network, gamma, lam, scaler, iteration, state_dict, logger, states= None):
def advantage_fun(trajectories, gamma, lam, scaler, iteration, state_dict, val_func, logger, L):
    """
...
if __name__ == "__main__":
    ...
    parser.add_argument('-n', '--num_policy_iterations', type=int, help='Number of policy iterations to run',
                        default=25) #default=50, use 5 for val fun comp.
