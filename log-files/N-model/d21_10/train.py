...
def adv_logger(adv, trajectory, adv_log, state = np.array([2,2]), length = 500):
    """
...
def advantage_fun(trajectories, gamma, lam, scaler, iteration, adv_log, val_func, logger, L):
    ...
    # for key, val in lst1:
    #     logger.log({key:val})
    adv_log = adv_logger(advantages, trajectory, adv_log) 
...
def build_train_set(trajectories, gamma, scaler, state_dict, logger, L):
    """
...
def main(network, num_policy_iterations, no_of_actors, episode_duration, no_arrivals, gamma, lam, clipping_parameter,
    ...
    while iteration < num_policy_iterations:
        ...
        add_value(trajectories, val_func, scaler, network.next_state_list())
        # compute advantage function estimates  
        # actions, advantages = build_train_set(trajectories, gamma, scaler, state1_dict, logger, L)
    ...
    np.savetxt(str(logger.path_adv) + "/adv_estimates.csv", adv_log, delimiter=",", fmt='%s')
