...
def add_disc_sum_rew(trajectories, policy, network, gamma, lam, scaler, iteration, state1_dict, logger, states = None):
    ...
    for trajectory in trajectories:
        ...
        if iteration!=1:
            ...
            log_vals(disc_sum_rew, 'val1_', states, logger,  True )
...
def main(network, num_policy_iterations, no_of_actors, episode_duration, no_arrivals, gamma, lam, clipping_parameter,
    ...
    while iteration < num_policy_iterations:
        ...
        lst1, advantages = advantage_fun(trajectories, policy, network, gamma, lam, scaler, iteration, state1_dict, logger, states)
        lst = build_train_set2(trajectories, gamma, scaler, state2_dict, logger, states) #use algo 1 advantage function 

        j = 0 #counter 
        for i in states:
            logger.log({'diff_' + str(i) : lst[j] - lst1[j]})
            j +=1
